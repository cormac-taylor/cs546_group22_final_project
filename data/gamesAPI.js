...
export const searchGamesByTitle = async (title) => {
  const { data } = await axios.get(
    `https://boardgamegeek.com/xmlapi/search?search=${title}`
  );
  ...
  if (Array.isArray(obj.boardgames.boardgame)) {
    ...
    // api does not return a list if only a single item is found, so this check is to ensure a list is returned
    return [obj.boardgames.boardgame];
  }
};
...
export const searchGamesByID = async (id) => {
  ...
  const { data } = await axios.get(
    `https://boardgamegeek.com/xmlapi/boardgame/${id}`
  );
